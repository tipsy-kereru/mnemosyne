#!/usr/bin/env bash
# Build recipe for the `slm` extension payload (ISSUE-0007 / SPEC-PACKAGE-001).
#
# Assembles GLiNER2 + REBEL + CPU torch into a payload directory, computes
# per-file SHA256, and emits the canonical manifest.json. The output is a
# platform/python-tagged tarball + manifest suitable for attaching to a
# GitHub Release on tipsy-kereru/mnemosyne-ext-slm.
#
...
#
# Output:
#   dist/slm-<version>-<platform>-<python_tag>.tar.gz
#   dist/manifest.json
set -euo pipefail
...
# Detect platform + python tag (mirrors installer._detect_platform).
detect_platform() {
    local os machine
...
# Stage 1: install the heavy deps into an isolated venv, then copy the
# importable packages into the payload dir. CPU-only torch avoids the CUDA
# wheel bloat (REQ: NO heavy wheels in repo).
python -m venv "${work_dir}/venv"
# shellcheck disable=SC1091
source "${work_dir}/venv/bin/activate"
pip install --upgrade pip --quiet
# CPU-only torch: the default PyPI wheel bundles CUDA and is ~2GB. We pull
# the CPU index instead. GLiNER2 + REBEL ride on torch + transformers.
pip install --index-url https://download.pytorch.org/whl/cpu --quiet \
    torch --no-deps
pip install --quiet \
    gliner \
    transformers \
...
for pkg in torch gliner transformers sentencepiece; do
...
# Stage 4: tar up the payload (manifest ships as a separate release asset,
# not inside the tarball, so the installer can regenerate it).
