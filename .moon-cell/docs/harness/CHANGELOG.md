...
### Dependencies
```
SPEC-JOPLIN-001 (API) → SPEC-JOPLIN-002 (Bridge) → SPEC-JOPLIN-003 (Viz)
                                              └─→ SPEC-JOPLIN-004 (Sync)
```

### Evidence
- Kuku project analysis informed feature parity decisions.
- Joplin plugin API capabilities verified (child_process, panels, content_scripts).
- mnemosyne KnowledgeGraph class has all needed CRUD methods.

### Preserved
- No existing product code changed.
- All 507 tests unaffected.
- Existing `.moon-cell/` artifacts patched, not regenerated.

## [0.2.9] - 2026-05-30

### Audit
- Full Moon Cell audit against git history and codebase state.
- Reconciled 11 discrepancies between MANIFEST and actual project state.
- SPEC-ARCH-ASYNC-001 promoted from planned to completed (T3/T4 implemented 2026-05-05, commit 746be8b).
- SPEC-PROJECT-REGISTRY-001 added to MANIFEST (implemented 2026-05-07, commit 9f7507a).
- Added "Untracked Features" section for skill install/update, hook install, z.ai provider, GLM fixes.
- Updated evidence: 507 tests pass, 46 source files, 32 test files, package v0.2.0.
- Updated VERSION timestamp, CONTEXT_HANDOFF, TASK_ROUTING.

### Evidence
- `uv run pytest -q` → 507 passed.
- `ruff check mnemosyne tests scripts/bench/` → All checks passed.
- Git log inspected: 25 commits since last MANIFEST update (2026-05-04).
- 22 completed SPECs, 0 planned, 1 candidate.

## [0.2.8] - 2026-05-07

### Completed
- SPEC-PROJECT-REGISTRY-001: Project-Scoped Knowledge Graph with Hash-Based Registry.
- Added `projects` table to knowledge.db (hash-based project ID, scope linkage).
- Added `mnemosyne/project/project.py` with auto-detection (CWD → .git/pyproject.toml/go.mod markers).
- Added `mnemosyne project` subcommand: list, show, register, unregister, migrate.
- Auto-registration on `mnemosyne add`/`update` when in project directory.
- `mnemosyne skill update` subcommand added.

### Evidence
- Commit 9f7507a: project registry implementation.
- Commit 8ae7e9d: skill update subcommand.
- DB tables confirmed: entities, relations, scopes, projects, ingest_cache.

## [0.2.7] - 2026-05-04 to 2026-05-06

### Completed
- SPEC-ARCH-ASYNC-001 T3/T4: Async I/O methods for URL fetch and LLM extraction.
- Added z.ai GLM provider (OpenAI-compatible LLM bridge).
- Added `mnemosyne skill install` subcommand.
- Added `mnemosyne hook install` CLI command for auto-sync (git, claude, codex, gemini, copilot).
- Package version bumped to 0.2.0.
- Multiple GLM response quality fixes (JSON mode, preamble suppression, max_tokens).
- Benchmark results updated to 921 pages baseline.

### Evidence
- Commit 746be8b: async I/O implementation.
- Commit eba5c64: z.ai provider.
- Commit c153d8a: skill install.
- Commit 2411ecd: hook install.
- Commit 569c4d1: version 0.2.0.
- Benchmark B2 LLM batch: PASS (187.38s > 60s threshold).
