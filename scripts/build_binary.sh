...
bake_version() {
    ...
    # at runtime and __init__ falls back. Bake the RELEASE version into a
    # module the binary ships, so 'mnemosyne --version' reports the real tag.
    # Prefer the release tag (GITHUB_REF_NAME for tag pushes, e.g. v0.5.7 ->
    # 0.5.7), then git describe, then pyproject as a last resort.
    local ver=""
    if [[ -n "${GITHUB_REF_NAME:-}" && "${GITHUB_REF_NAME}" == v* ]]; then
        ver="${GITHUB_REF_NAME#v}"
    fi
    if [[ -z "${ver}" ]] && command -v git >/dev/null 2>&1; then
        ver="$(cd "${REPO_ROOT}" && git describe --tags --abbrev=0 2>/dev/null || true)"
        ver="${ver#v}"
    fi
    if [[ -z "${ver}" ]]; then
    ...
    if [[ -z "${ver}" ]]; then
        err "could not determine version (GITHUB_REF_NAME / git describe / pyproject all failed)"
        return 1
    fi
    printf '# AUTO-GENERATED by scripts/build_binary.sh — do not edit.\n__version__ = "%s"\n' "${ver}" \
...
pathlib.Path(sys.argv[2]).write_text(
    "# AUTO-GENERATED by scripts/build_binary.sh — do not edit.\n"
    + "SKILL_MD = " + json.dumps(text) + "\n",
    encoding="utf-8",
)
PY
