...
#
# Linux/macOS PyOxidizer builds. Windows uses scripts/build_windows.py.
#
...
build_dependency_venv() {
    ...
    local venv_sp_dir="${VENV_DIR}/lib/python${PYTHON_VERSION}/site-packages"
    ...
    # Must match the absolute paths in FS_FILES.
...
bake_version() {
    ...
    # Source metadata is authoritative, including on pre-tag diagnostic builds.
    # A nearest-ancestor git tag would silently bake an older version.
    local ver
    ver="$(grep -m1 '^version' "${REPO_ROOT}/pyproject.toml" | sed -E 's/.*"([^"]+)".*/\1/')"
    if [[ "${GITHUB_REF_TYPE:-}" == tag && "${GITHUB_REF_NAME:-}" != "v${ver}" ]]; then
        err "release tag ${GITHUB_REF_NAME} does not match source version ${ver}"
        return 1
    fi
    if [[ -z "${ver}" ]]; then
...
install_binary() {
    ...
    # Linux binutils strips ELF; macOS strip removes local symbols from Mach-O.
